      ...
      <div className="relative z-10 pt-32 pb-20 px-4 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6 flex-wrap">
          ...
          <div className="flex items-center gap-4">
            <span className="text-purple-300">Welcome, {user?.attributes?.email || user?.username}</span>
              ...
              {requests.length === 0 ? (
                ...
                requests.map(r => (
                  <tr key={r.id} className="border-b border-purple-500/20">
                    ...
                    <td>
                      {r.status === 'pending' && <button onClick={()=>updateStatus(r.id,'in-progress')} className="bg-blue-600 px-2 py-1 rounded text-xs">Start</button>}
